...
d3.json(dataUrl, function(nations) {
	...
	var year_idx = Number(document.getElementById("year_slider").value) - 1960;

	// Calculate the averages for each region.
	var region_names = ["Sub-Saharan Africa", "South Asia", "Middle East & North Africa", "America", "East Asia & Pacific", "Europe & Central Asia", "World", "Latin America & Caribbean"];
	...
	var margin = {top: 19.5, right: 19.5, bottom: 25.5, left: 39.5};
	var frame_width = 1360;
	...
	var xScale = d3.scaleLinear(); // population above 65
	xScale.domain([0, 28]);
	...
	var yScale = d3.scaleLinear().domain([0, 100]).range([canvas_height, 0]);  // life expectancy
	...
	var rScale = d3.scaleSqrt().domain([0, 20]).range([0, 40]); // fertility rate
    ...
    .attr("x", canvas_width)
    .attr("y", - 6)
    .text("Population ages 65 and above (% of total)");
	...
	d3.select("#year_slider").on("input", function () {
		year_idx = Number(this.value) - 1960;
		update();
	});
